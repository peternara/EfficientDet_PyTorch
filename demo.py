...
class Detect(object):
    ...
    def process(self, file_name=None, img=None, show=False):
        ...
        with torch.no_grad():
            ...
            regression, classification  = self.model(img)
            scores, classify, bbox = models.efficientdet.detection(classification, regression, img)
            print("last result: {},   {},   {}".format(scores.shape, classify.shape, bbox.shape))

            bboxes = list()
            labels = list()
            bbox_scores = list()
            for j in range(scores.shape[0]):
                bbox = bbox[[j], :][0].data.cpu().numpy()
                ...
                label_name = VOC_CLASSES[int(classify[[j]])]
...
if __name__ == '__main__':
    ...
    detect = Detect(weights = "E:/Tengine2_0/Code/EfficientNet/EfficientDet_PyTorch/checkpoint_48.pth",
                    num_class = 20)
